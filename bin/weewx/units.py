...
class ValueHelper(object):
    ...
    def toString(self, addLabel=True, useThisFormat=None, NONE_string=None,
                 localize=True, fmt='json', delimiter=','):
        ...
        if hasattr(vtx.value, '__iter__'):
            # It's a vector
            # Initialise an empty list to hold the result elements
            _result = []
            ...
            if fmt == 'delimited':
                # Delimited, so get a string of the elements separated with the
                # delimiter
    ...
    def string(self, NONE_string=None, fmt=None, delimiter=','):
        ...
        if fmt == 'delimited':
            ...
            return self.toString(NONE_string='null',
                                 fmt=fmt)
        else:
            return self.toString(NONE_string=NONE_string)
    ...
    def nolabel(self, format_string, NONE_string=None, fmt=None, delimiter=','):
        """Returns a formatted version of the datum, using a user-supplied
        format. No label."""
        if fmt == 'delimited':
            return self.toString(addLabel=False,
                                 useThisFormat=format_string,
                                 NONE_string=NONE_string,
                                 fmt=fmt,
                                 delimiter=delimiter)
        elif fmt == 'json':
    ...
    def ordinal_compass(self, fmt=None, delimiter=','):
        """Returns an ordinal compass direction (eg, 'NNW')"""

        # Get the raw value tuple
        vtx = self._raw_value_tuple
        # Is our datum a vector, if format each element separately
        if hasattr(vtx.value, '__iter__'):
            # It's a vector
            ...
            for elm in vtx.value:
                ...
                _vt = weewx.units.ValueTuple(elm, self.value_t.unit,
                                             self.value_t.group)
                # Ask the formatter to look up an appropriate ordinate for the
                # element
                _e = self.formatter.to_ordinal_compass(_vt)
                # Add the formatted element to our result list
                _result.append(_e)
            # Is fmt is specified, if so we only know about delimited and json
            if fmt == 'delimited':
            ...
            # ordinate
            s = self.formatter.to_ordinal_compass(vtx)
        # Return the resulting string
        return s
    ...
    def json(self):
        ...
        vtx = self._raw_value_tuple
        return json.loads(json.dumps(vtx.value))
