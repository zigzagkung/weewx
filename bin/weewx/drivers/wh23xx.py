...
class WH23xxDriver(weewx.drivers.AbstractDevice):
    ...
    def _get_current(self):
        ntries = 0
        while ntries < self._max_tries:
            ntries += 1
            try:
                return self._station.get_current()
            except weewx.WeeWxIOError, e:
                logerr("get_current: failed attempt %d of %d: %s" %
                       (ntries, self._max_tries, e))
            time.sleep(self._retry_wait)
        msg = "read failed: max retries (%d) exceeded" % self._max_tries
        logerr(msg)
...
class WH23xxStation(weeusb.USBHID):
    ...
    def __init__(self):
        super(WH23xxStation, self).__init__(0x10c4, 0x8468)

    def write(self, label, buf):
        logdbg("write: %s: %s" % (label, _fmt(buf)))
        cnt = self._write(buf)
        if cnt != len(buf):
    ...
    def _read_eeprom(self, addr, size):
        ...
        buf.extend(cmd)
        buf.append(chksum)
        self.write("read_eeprom", buf)

        # now do the actual read.
        buf = self._read(self.USB_PACKET_SIZE, endpoint_in=self.USB_ENDPOINT_IN)
        if not buf:
...
#
# PYTHONPATH=bin python bin/weewx/drivers/wh23xx.py
